...
import tech.pegasys.pantheon.ethereum.p2p.discovery.internal.PeerDistanceCalculator;
...
//  }
    ...
    public void dispatchPing(final MockPeerDiscoveryAgent fromAgent, final MockPeerDiscoveryAgent toAgent, final Packet packet) {
        RecursivePeerRefreshState recursivePeerRefreshState = fromAgent.getPeerDiscoveryController().get().getRecursivePeerRefreshState();
        SortedMap<BytesValue, RecursivePeerRefreshState.MetadataPeer> oneTrueMap = recursivePeerRefreshState.getOneTrueMap();
        final RecursivePeerRefreshState.MetadataPeer iterationParticipant = new RecursivePeerRefreshState.MetadataPeer(toAgent.getAdvertisedPeer(), distance(recursivePeerRefreshState.getTarget(), toAgent.getAdvertisedPeer().getId()));
        oneTrueMap.put(toAgent.getAdvertisedPeer().getId(), iterationParticipant);

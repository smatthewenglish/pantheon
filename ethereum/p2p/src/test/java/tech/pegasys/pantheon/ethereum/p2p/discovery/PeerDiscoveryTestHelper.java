...
public class PeerDiscoveryTestHelper {
  ...
  public void dispatchPing(
    ...
    final RecursivePeerRefreshState recursivePeerRefreshState =
        fromAgent.getPeerDiscoveryController().get().getRecursivePeerRefreshState();

    final SortedMap<BytesValue, RecursivePeerRefreshState.MetadataPeer> oneTrueMap =
        recursivePeerRefreshState.getOneTrueMap();

    final RecursivePeerRefreshState.MetadataPeer iterationParticipant =
        new RecursivePeerRefreshState.MetadataPeer(
            toAgent.getAdvertisedPeer(),
            distance(recursivePeerRefreshState.getTarget(), toAgent.getAdvertisedPeer().getId()));

...
public class PeerDiscoveryControllerTest {
  ...
  public void bootstrapPeersPongReceived_HashUnmatched() {
    ...
    // Send a PONG packet from peer 1.
    final PongPacketData packetData =
        PongPacketData.create(localPeer.getEndpoint(), mockPacket.getHash());
  ...
  public void findNeighborsSentAfterBondingFinished() {
    ...
    assertThat(data.getTarget()).isEqualTo(controller.getTarget());

    assertThat(controller.getPeers()).hasSize(1);
    assertThat(controller.getPeers().stream().findFirst().get().getStatus())
        .isEqualTo(DiscoveryPeerStatus.DISPATCHED_FIND_NEIGHBOURS_TO);
  }
  ...
  public void peerSeenTwice() throws InterruptedException {
    ...
    final PingPacketData pingPacketData2 =
        PingPacketData.create(localPeer.getEndpoint(), peers.get(2).getEndpoint());
    final Packet pingPacket2 = Packet.create(PacketType.PING, pingPacketData2, keyPairs.get(2));
    final PongPacketData pongPacketData2 =
        PongPacketData.create(localPeer.getEndpoint(), pingPacket2.getHash());
        ...
        .filteredOn(
            p ->
                p.equals(peers.get(2))
                    && p.getStatus() == DiscoveryPeerStatus.DISPATCHED_FIND_NEIGHBOURS_TO)
        .hasSize(1);
  }

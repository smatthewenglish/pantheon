...
public class PeerDiscoveryControllerTest {
    ...
    public void bootstrapPeersRetriesSent() {
        ...
        controller
                .getPeers()
                .forEach(
                        p -> assertThat(DiscoveryPeerStatus.Lifecycle.bonded.contains(p.getStatus())).isTrue());
    }
    ...
    public void bootstrapPeersPongReceived_HashMatched() {
        ...
        assertThat(
                ...
                .hasSize(2);
        assertThat(
                controller.getPeers().stream().filter(p -> p.getStatus() == DiscoveryPeerStatus.RECEIVED_PONG_FROM))
                .hasSize(1);
    }
    ...
    public void peerSeenTwice() throws InterruptedException {
        ...
        // Simulate a PONG message(s).
        final PingPacketData pingPacketData0 =
                PingPacketData.create(localPeer.getEndpoint(), peers.get(0).getEndpoint());
        final Packet pingPacket0 = Packet.create(PacketType.PING, pingPacketData0, keyPairs.get(0));
        final PongPacketData pongPacketData0 =
                PongPacketData.create(localPeer.getEndpoint(), pingPacket0.getHash());
        final Packet pongPacket0 = Packet.create(PacketType.PONG, pongPacketData0, keyPairs.get(0));
        controller.onMessage(pongPacket0, peers.get(0));
        /* * */
        final PingPacketData pingPacketData1 =
                PingPacketData.create(localPeer.getEndpoint(), peers.get(1).getEndpoint());
        final Packet pingPacket1 = Packet.create(PacketType.PING, pingPacketData1, keyPairs.get(1));
        final PongPacketData pongPacketData1 =
                PongPacketData.create(localPeer.getEndpoint(), pingPacket1.getHash());
        final Packet pongPacket1 = Packet.create(PacketType.PONG, pongPacketData1, keyPairs.get(1));
        ...
        final Packet neighborsPacket0 = Packet.create(PacketType.NEIGHBORS, neighbors0, keyPairs.get(0));
        ...
        final Packet neighborsPacket1 = Packet.create(PacketType.NEIGHBORS, neighbors1, keyPairs.get(1));
        controller.onMessage(neighborsPacket1, peers.get(1));

        // Assert that we're "bonding" with peers.
        assertThat(controller.getPeers()).hasSize(2);
        assertThat(controller.getPeers())
                .filteredOn(p -> p.getStatus() == DiscoveryPeerStatus.RECEIVED_NEIGHBOURS_FROM)
                .hasSize(2);
        ...
        final PingPacketData pingPacketData2 = PingPacketData.create(localPeer.getEndpoint(), peers.get(2).getEndpoint());
        final Packet pingPacket2 = Packet.create(PacketType.PING, pingPacketData2, keyPairs.get(2));
        final PongPacketData pongPacketData2 = PongPacketData.create(localPeer.getEndpoint(), pingPacket2.getHash());
        final Packet pongPacket2 = Packet.create(PacketType.PONG, pongPacketData2, keyPairs.get(2));
    ...
    public void shouldNotBondWithBlacklistedPeer()
        ...
        verify(controller, times(0)).dispatchPing(otherPeer);
        verify(controller, times(1)).dispatchPing(otherPeer2);
    }
